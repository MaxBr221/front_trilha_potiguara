        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {palavrasFiltradas.length > 0 ? (
            palavrasFiltradas.map(palavra => (
              <div 
                key={palavra.id} 
                className="bg-white dark:bg-stone-900 p-5 rounded-2xl border border-stone-200 dark:border-stone-800 hover:border-primary/50 dark:hover:border-primary/50 hover:shadow-md transition-all group relative cursor-pointer"
                onClick={() => copiarPalavra(palavra.palavraTupi)}
                title="Clique para copiar"
              >
                <div className="absolute top-4 right-4 text-stone-300 dark:text-stone-600 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <div className="flex justify-between items-start mb-2 pr-8">
                  <div className="flex items-center gap-2">
                    ...
                    <button 
                      ...
                      className="p-1.5 text-stone-400 dark:text-stone-500 hover:text-primary dark:hover:text-primary hover:bg-primary/10 rounded-lg transition-colors"
                      title="Ouvir pronúncia"
                    >
                      <Volume2 className="w-5 h-5" />
                    </button>
                  </div>
                  <span className="text-[10px] uppercase font-bold tracking-wider px-2 py-1 bg-stone-100 dark:bg-stone-800 text-stone-500 dark:text-stone-400 rounded">
                    {palavra.tipo}
                  </span>
                </div>
                <p className="text-stone-700 dark:text-stone-300 font-medium mb-3">{palavra.traducaoPtBr}</p>
              </div>
            ))
          ) : (
